...
public class CleanupTaskHistory extends CleanupTaskModel {
    ...
    private JocClusterServiceTaskState cleanupOrders(TaskDateTime dateTime, boolean deleteLogs) throws SOSHibernateException {
        ...
        if (isCompleted(state)) {
            LocalDateTime remainingStartTime = getRemainingStartTime(dateTime);
    ...
    protected JocClusterServiceTaskState cleanupOrders(Scope scope, Range range, LocalDateTime startTime, String ageInfo, boolean deleteLogs)
    ...
    private JocClusterServiceTaskState cleanupRemaining(LocalDateTime startTime, String ageInfo, boolean deleteLogs) throws SOSHibernateException {
        if (isStopped()) {
            return JocClusterServiceTaskState.UNCOMPLETED;
        }
        tryOpenSession();
    ...
    private Long getOrderMaxMainParentId(Scope scope, Range range, LocalDateTime startTime, String ageInfo) throws SOSHibernateException {
    ...
    private Long getOrderLogsMaxMainParentId(Scope scope, Range range, LocalDateTime startTime, String ageInfo) throws SOSHibernateException {
    ...
    private boolean cleanupOrderTags(LocalDateTime startTime) throws SOSHibernateException {
    ...
    private CleanupPartialResult deleteOrderTags(LocalDateTime date) throws SOSHibernateException {
    ...
    private boolean cleanupOrders(Scope scope, Range range, LocalDateTime startTime, String ageInfo, boolean deleteLogs, Long maxMainParentId)
            throws SOSHibernateException {
    ...
    private boolean deleteRemainingStates(LocalDateTime startTime, String ageInfo) throws SOSHibernateException {
        StringBuilder log = new StringBuilder("[").append(getIdentifier()).append("][");
    ...
    private boolean deleteRemainingLogs(LocalDateTime startTime, String ageInfo) throws SOSHibernateException {
        StringBuilder log = new StringBuilder("[").append(getIdentifier()).append("][");
    ...
    private void deleteControllersAndAgents(LocalDateTime startTime, String ageInfo) throws SOSHibernateException {
    ...
    protected StringBuilder deleteControllers(LocalDateTime readyTime, StringBuilder log) throws SOSHibernateException {
        ...
        query.setParameter("readyTime", SOSDate.toUtcDate(readyTime));
        int r = getDbLayer().getSession().executeUpdate(query);
    ...
    protected StringBuilder deleteAgents(LocalDateTime readyTime, StringBuilder log) throws SOSHibernateException {
        // - delete all older than the readyTime
        ...
        if (Files.exists(dir)) {
            try {
                if (SOSPath.isDirectoryEmpty(dir)) {
                    ...
                    int i = 0;
                    BiPredicate<Path, BasicFileAttributes> predicate = (path, attributes) -> attributes.isRegularFile() && (SOSDate.toUtcEpochMilli(
                            datetime.getDatetime()) > attributes.lastModifiedTime().toMillis());
