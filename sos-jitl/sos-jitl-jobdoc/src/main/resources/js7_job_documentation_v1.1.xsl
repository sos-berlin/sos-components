...
<xsl:stylesheet version="1.0"
	...
	<xsl:output method="xml" omit-xml-declaration="no"
		version="1.0" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN"
	...
	<xsl:template match="/jobdoc:description">
		<html>
			...
			<body>
				...
				<xsl:for-each
					select="//jobdoc:*[local-name() = 'documentation' or local-name() = 'note']/@language[(count(. | key('languages', .)[1]) = 1)]">
					<xsl:apply-templates
						select="/jobdoc:description" mode="main">
	...
	<xsl:template match="/jobdoc:description" mode="main">
		...
		<div>
			...
			<xsl:choose>
				<xsl:when
					test="jobdoc:documentation[not (@language) or @language=$lang]">
					<xsl:apply-templates
						select="jobdoc:documentation[not (@language) or @language=$lang]">
						<xsl:with-param name="lang" select="$lang" />
					</xsl:apply-templates>
				</xsl:when>
			</xsl:choose>
	...
	<xsl:template match="jobdoc:job" mode="genXML">
		<span class="code">
			...
			&lt;/description&gt;
			<xsl:if
				test="../jobdoc:configuration/jobdoc:params[not(@id='return_parameter')]">
				<br />
				&#160;&#160;&#160;&#160;
				<br />
				&#160;&#160;&#160;&#160;
				&lt;params&gt;
				<xsl:apply-templates
					select="../jobdoc:configuration/jobdoc:params[not(@id='return_parameter')]"
					mode="genXML" />
				<br />
				&#160;&#160;&#160;&#160;
				&lt;/params&gt;
			...
			<td class="td2">
				<xsl:choose>
					<xsl:when test="ancestor::jobdoc:monitor">
						<!--xsl:attribute name="onmouseover">showWMTT('ttMonitorScript_<xsl:value-of 
							select="$lang"/>')</xsl:attribute -->
						<xsl:call-template name="get_label">
							<xsl:with-param name="key"
								select="'MonitorScript'" />
							<xsl:with-param name="lang" select="$lang" />
					...
					<xsl:otherwise>
						<!--xsl:attribute name="onmouseover">showWMTT('ttScript_<xsl:value-of 
							select="$lang"/>')</xsl:attribute -->
						<xsl:call-template name="get_label">
	...
	<xsl:template match="jobdoc:releases">
		<xsl:param name="lang" />
		<xsl:variable name="cnt_releases"
			select="count(jobdoc:release)" />
		<xsl:if test="$cnt_releases = 1">
			<table class="box" style="{$DisplayStyle}">
				<xsl:attribute name="id">firstRelease_<xsl:value-of
					select="$lang" /></xsl:attribute>
		...
		<xsl:if test="$cnt_releases > 1">
			<table class="box" style="{$DisplayStyle}">
				...
				<tr>
					...
					<td>
						<a class="doc">
							<xsl:attribute name="id">releaseLink_<xsl:value-of
								select="$lang" /></xsl:attribute>
							<xsl:attribute name="href">javascript:showPreviousReleases('<xsl:value-of
								select="$lang" />');</xsl:attribute>
							...
							<xsl:call-template name="get_label">
								...
								<xsl:with-param name="label_de"
									select="'Ältere Releases anzeigen'" />
								<xsl:with-param name="label_en"
									select="'Show previous Releases'" />
							</xsl:call-template>
						</a>
					</td>
				</tr>
			...
			<table class="boxRelease" style="{$DisplayStyle}">
				<xsl:attribute name="id">previousRelease_<xsl:value-of
					select="$lang" /></xsl:attribute>
				<xsl:for-each select="jobdoc:release">
					<xsl:if test="position() > 1">
						<xsl:call-template name="jobdoc:release_x">
	...
	<xsl:template name="jobdoc:release_x">
		...
		<xsl:apply-templates
			select="jobdoc:changes[@language=$lang]" />
	...
	<xsl:template match="jobdoc:resources">
		<xsl:param name="lang" />
		<table class="box" style="{$DisplayStyle}">
			<xsl:attribute name="id">resources_<xsl:value-of
				select="$lang" /></xsl:attribute>
			<tr>
				<td class="td1">
					<span class="section">
	...
	<xsl:template match="jobdoc:database">
		<xsl:param name="lang" />
		<!-- <xsl:if test="@required='true'"> A Database is required<br/> </xsl:if> 
			<xsl:if test="@required='false'"> This job can be run with a database<br/> 
			</xsl:if> -->
	...
	<xsl:template match="jobdoc:configuration">
		...
		<xsl:if test="child::*">
			<table class="box" style="{$DisplayStyle}">
				<xsl:attribute name="id">configuration_<xsl:value-of
					select="$lang" /></xsl:attribute>
				<tr>
					<td class="td1">
						<span class="section">
	...
	<xsl:template
		match="jobdoc:params[parent::jobdoc:configuration] | jobdoc:params[parent::jobdoc:params]">
		<xsl:param name="lang" />
		<tr>
			<td class="td1">
					...
					<xsl:otherwise>
						...
						<xsl:if test="child::*">
							<table class="section" cellpadding="0" cellspacing="1">
								<xsl:apply-templates
									select="codeexample|jobdoc:codeexample|xi:include|jobdoc:param|jobdoc:params/jobdoc:*[not(@language) or @language = $lang]">
									<xsl:with-param name="lang" select="$lang" />
								</xsl:apply-templates>
							</table>
						</xsl:if>
	...
	<xsl:template match="jobdoc:params" mode="reference">
		...
		<xsl:variable name="reftext">
			<xsl:choose>
				<xsl:when test="ancestor::jobdoc:payload">
					<xsl:call-template name="get_label">
						<xsl:with-param name="key"
							select="'PayloadParameter'" />
						<xsl:with-param name="lang" select="$lang" />
						<xsl:with-param name="label_de"
							select="'Payload-Parameter'" />
						<xsl:with-param name="label_en"
							select="'Payload Parameter'" />
					</xsl:call-template>
				</xsl:when>
		...
		<a class="doc">
			<xsl:attribute name="href">#<xsl:value-of
				select="@id" />_<xsl:value-of select="$lang" /></xsl:attribute>
			<span style="font-family:Arial;font-size:12px;">&#8594;</span>
			<xsl:text>&#160;</xsl:text>
				...
				<td class="section2">
					<span class="desc">
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:variable name="defVal">
									<xsl:choose>
										<xsl:when
											test="@default_value and not(@default_value='')">
											<xsl:value-of select="@default_value" />
										</xsl:when>
										<xsl:when
											test="@DefaultValue and not(@DefaultValue='')">
						...
						<xsl:if test="@since and @since != ''">
							<p>
								<xsl:value-of
									select="concat('This parameter is introduced with version ', @since)" />
							</p>
						...
						<xsl:choose>
							<xsl:when
								test="$extdoc and $extdoc//jobdoc:note[@language=$lang]">
								<xsl:apply-templates
									select="$extdoc//jobdoc:note[@language=$lang] | $extdoc//note[@language=$lang] | $extdoc//jobdoc:codeexample">
									<xsl:with-param name="lang" select="$lang" />
								</xsl:apply-templates>
							</xsl:when>
							<xsl:when test="jobdoc:note">
					...
					<!-- <xsl:apply-templates select="xi:include|jobdoc:examples|jobdoc:codeexample"> 
						<xsl:with-param name="lang" select="$lang"/> </xsl:apply-templates> -->
				</td>
			</tr>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="jobdoc:param" mode="reference">
		...
		<a class="doc">
			<xsl:attribute name="href">#<xsl:value-of
				select="@id" />_<xsl:value-of select="$lang" /></xsl:attribute>
			<xsl:attribute name="href">#<xsl:value-of
				select="@name" />_<xsl:value-of select="$lang" /></xsl:attribute>
			<span style="font-family:Arial;font-size:12px;">&#8594;</span>
			<xsl:text>&#160;</xsl:text>
			<xsl:value-of select="$reftext" />
			<xsl:text>&#160;</xsl:text>
	...
	<xsl:template match="jobdoc:params" mode="genXML">
		<xsl:apply-templates
			select="jobdoc:params | jobdoc:param | xi:include" mode="genXML" />
	</xsl:template>
	<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	...
	<xsl:template match="jobdoc:payload">
		...
		<xsl:call-template name="set_anchor">
			<xsl:with-param name="lang" select="$lang" />
			<xsl:with-param name="anchor_name"
				select="jobdoc:params/@id" />
		</xsl:call-template>
		<tr>
			<td class="td1">
				<xsl:text>&#160;</xsl:text>
			...
			<td class="td2">
				<!--td class="td2" onmouseout="hideWMTT()" > <xsl:attribute name="onmouseover">showWMTT('ttpayload_<xsl:value-of 
					select="$lang"/>')</xsl:attribute -->
				<xsl:call-template name="get_label">
				...
				<!--span class="tooltip"> <xsl:attribute name="id">ttpayload_<xsl:value-of 
					select="$lang"/></xsl:attribute> <xsl:call-template name="get_label"> <xsl:with-param 
					name="key" select="'Payload.tooltip'"/> <xsl:with-param name="lang" select="$lang"/> 
					<xsl:with-param name="label_de" select="'Auftragsgesteuerte Jobs verarbeiten 
					Aufträge mit einer optionalen Payload für auftragsbezogene Parameter.'"/> 
					<xsl:with-param name="label_en" select="'Order driven jobs execute orders 
					with an optional payload that contains parameters for the order.'"/> </xsl:call-template> 
					</span -->
			</td>
			<td class="td3">
				<xsl:choose>
	...
	<xsl:template
		match="jobdoc:params[parent::jobdoc:payload]">
		<xsl:param name="lang" />
		<xsl:if test="@reference and not(@reference='')">
			<xsl:call-template name="process_reference">
				<xsl:with-param name="lang" select="$lang" />
	...
	<xsl:template match="jobdoc:profile">
		...
		<tr>
			...
			<td class="td2">
				<!--td class="td2" onmouseout="hideWMTT()" > <xsl:attribute name="onmouseover">showWMTT('ttsettingsIni_<xsl:value-of 
					select="$lang"/>')</xsl:attribute -->
				<xsl:call-template name="get_label">
				...
				<!--span class="tooltip"> <xsl:attribute name="id">ttsettingsIni_<xsl:value-of 
					select="$lang"/></xsl:attribute> <xsl:call-template name="get_label"> <xsl:with-param 
					name="key" select="'Settings.tooltip'"/> <xsl:with-param name="lang" select="$lang"/> 
					<xsl:with-param name="label_de" select="'Zentrale Einstellungen für den Job 
					in einer Konfigurationsdatei. Per Default ist der Name der Konfigurationsdatei 
					factory.ini .'"/> <xsl:with-param name="label_en" select="'Central job settings 
					given in a profile: the default profile is held in the factory.ini configuration 
					file .'"/> </xsl:call-template> </span -->
			</td>
			<td class="td3">
				<xsl:if test="jobdoc:note">
					<xsl:call-template name="call_notes">
				...
				<xsl:call-template name="get_label">
					<xsl:with-param name="key"
						select="'ConfigurationFileName'" />
					<xsl:with-param name="lang" select="$lang" />
					<xsl:with-param name="label_de"
						select="'Name der Konfigurationsdatei'" />
					<xsl:with-param name="label_en"
						select="'Name of Configuration File'" />
				</xsl:call-template>
				<xsl:text>: </xsl:text>
				<xsl:if test="@name">
					<span class="sourceName">
		...
		<tr>
			...
			<td class="td2">
				<!--td class="td2" onmouseover="showWMTT('ttsettingsDB')" onmouseout="hideWMTT()" 
					> <xsl:attribute name="onmouseover">showWMTT('ttsettingsDB_<xsl:value-of 
					select="$lang"/>')</xsl:attribute -->
				<xsl:call-template name="get_label">
					<xsl:with-param name="key" select="'Database'" />
					<xsl:with-param name="lang" select="$lang" />
				...
				<!--span class="tooltip"> <xsl:attribute name="id">ttsettingsDB_<xsl:value-of 
					select="$lang"/></xsl:attribute> <xsl:call-template name="get_label"> <xsl:with-param 
					name="key" select="'Database.tooltip'"/> <xsl:with-param name="lang" select="$lang"/> 
					<xsl:with-param name="label_de" select="'Zentrale Einstellungen für den Job 
					in einer Datenbank. Alternativ zu den Einstellungen über XML-Konfiguration 
					oder Profile können Parameter auch aus logischen Tabellen in einer Datenbank 
					gelesen werden. Ein Parameter-Eintrag liegt innerhalb der logischen Struktur 
					der Attribute application/section/entry der Tabelle SETTINGS. Per Default 
					werden Einstellungen aus der Datenbank gelesen, die in der Konfigurationsdatei 
					factory.ini angegeben wurde.'"/> <xsl:with-param name="label_en" select="'Central 
					job settings read from a database. As an alternative to the XML configuration 
					file or the profile, settings can be read from database tables. Parameter 
					entries must use the attributes structure of the SETTINGS table - i.e. application/section/entry. 
					By default values are read from Job Scheduler Database whose connection data 
					is specified in the factory.ini configuration file.'"/> </xsl:call-template> 
					</span -->
			</td>
			<td class="td3">
				<xsl:if test="jobdoc:note">
	...
	<xsl:template name="set_anchor">
		...
		<a>
			<xsl:attribute name="name"><xsl:value-of
				select="$anchor_name" />_<xsl:value-of select="$lang" /></xsl:attribute>
		</a>
	</xsl:template>
	<xsl:template
		match="jobdoc:note | jobdoc:changes | jobdoc:documentation"
		mode="reference">
	...
	<xsl:template name="get_resource">
		...
		<a class="doc">
			<xsl:attribute name="onclick">show_div('resources','<xsl:value-of
				select="$lang" />');return false;</xsl:attribute>
			<xsl:attribute name="href">#<xsl:value-of
				select="$resource_id" /></xsl:attribute>
			<span style="font-family:Arial;font-size:12px;">&#8594;</span>
			<xsl:text>&#160;</xsl:text>
			<xsl:call-template name="get_label">
	...
	<xsl:template name="get_label">
		...
		<xsl:choose>
			<xsl:when
				test="$key and $lang_file and document($lang_file)">
	...
	<xsl:template name="i18n">
		...
		<xsl:variable name="i18n_key">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring-before($key,'.tooltip')" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="i18n_item"
			select="document($lang_file)/languages/item[@key=$i18n_key]" />
		<xsl:choose>
			<xsl:when
				test="not($tooltip) and not($i18n_item/language[@id=$lang]/text = '')">
				<xsl:copy-of
					select="$i18n_item/language[@id=$lang]/text/node()" />
			</xsl:when>
			<xsl:when
				test="not($tooltip) and not($lang = 'en') and not($i18n_item/language[@id='en']/text = '')">
				<xsl:copy-of
					select="$i18n_item/language[@id='en']/text/node()" />
			</xsl:when>
			<xsl:when
				test="$tooltip and not($i18n_item/language[@id=$lang]/tooltip = '')">
				<xsl:copy-of
					select="$i18n_item/language[@id=$lang]/tooltip/node()" />
			</xsl:when>
			<xsl:when
				test="$tooltip and not($lang = 'en') and not($i18n_item/language[@id='en']/tooltip = '')">
				<xsl:copy-of
					select="$i18n_item/language[@id='en']/tooltip/node()" />
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	...
	<!-- <xsl:template match="jobdoc:codeexample" > <code> <xsl:apply-templates 
		select="." /> </code> </xsl:template> -->

	<xsl:template
		match="jobdoc:codeexample | jobdoc:codeExample">
		<xsl:param name="lang" select="'en'" />
		<xsl:apply-templates
			select="jobdoc:title|title|jobdoc:explanation[@language=$lang]/jobdoc:title">
			<xsl:with-param name="lang" select="$lang" />
		</xsl:apply-templates>
		<!-- <xsl:apply-templates select="not (jobdoc:title|title)" > -->
		<xsl:apply-templates
			select="jobdoc:embeddedexample|jobdoc:embeddedExample|embeddedExample|embeddedexample | jobdoc:explanation">
			<xsl:with-param name="lang" select="$lang" />
		</xsl:apply-templates>
	...
	<xsl:template
		match="jobdoc:codeexample | jobdoc:codeExample" mode="copy">
		<xsl:param name="lang" select="'en'" />
		...
		<xsl:apply-templates
			select="jobdoc:embeddedexample|jobdoc:embeddedExample|embeddedExample | jobdoc:explanation">
			<xsl:with-param name="lang" select="$lang" />
		</xsl:apply-templates>
	</xsl:template>

	<xsl:template
		match="jobdoc:embeddedexample|jobdoc:embeddedExample">
		<xsl:param name="lang" select="'en'" />
		<pre class="example">
			<xsl:value-of select="." />
		</pre>
  ...
  var wmtt = null;
	...
	<xsl:template match="xi:include">
		...
		<xsl:if test="@parse = 'text'">
			<pre>
				<!-- XSLT 2.0 Function, not supported by many browsers <xsl:copy-of select="unparsed-text(@href)" 
					/> -->
	...
	<xsl:template match="jobdoc:wikilink | wikilink">
		...
		<xsl:value-of
				select="concat('http://', $lang, '.wikipedia.org/wiki/', $linkto)" />
	</xsl:attribute>
	...
	<xsl:template match="jobdoc:doxylink | doxylink">
		...
		<xsl:element name="a">
			<xsl:attribute name="href">
        <xsl:value-of
				select="concat('http://www.sos-berlin.com/doc/doxygen-docs/', @linkto, '/html/index.html')" />
    </xsl:attribute>
