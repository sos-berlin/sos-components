	...
	<xsl:template match="*" mode="copy_node">
		<xsl:choose>
			<xsl:when test="name()='xi:include'">
				...
				<xsl:apply-templates
					select="document(@href)/jobdoc:*" mode="copy_xiinclude" />
			</xsl:when>
			<xsl:when
				test="name() = 'params' and not(jobdoc:*) and not(xi:include)">
			</xsl:when>
			<xsl:when test=".//xi:include">
	...
	<xsl:template match="*" mode="copy_xiinclude">
		<xsl:element name="{name(.)}">
			...
			<xsl:if test="@DefaultValue">
				<xsl:attribute name="default_value"><xsl:value-of
					select="@DefaultValue" /></xsl:attribute>
			</xsl:if>
