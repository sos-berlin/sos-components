...
<xsl:stylesheet
	...
	<xsl:template match="/jobdoc:description">
		<xsl:processing-instruction name="xml-stylesheet">
			type="text/xsl" href="js7_job_documentation_v1.1.xsl"
		...
		<xsl:choose>
			<xsl:when test="count(.//jobdoc:params) &gt; 0">
				<description>
					<xsl:apply-templates
						select="child::*|@*|comment()" mode="copy_node" />
				</description>
			</xsl:when>
			<xsl:otherwise>
				<xsl:message>
	...
	<xsl:template match="jobdoc:configuration" mode="copy_node">
		<configuration>
			...
			<xsl:apply-templates select="jobdoc:note"
				mode="copy_node" />
			<xsl:apply-templates
				select="jobdoc:params[@id = 'return_parameter' and count(.//jobdoc:param) &gt; 0]"
				mode="copy_node" />
			<xsl:apply-templates
				select="jobdoc:params[not(@id = 'return_parameter') and count(.//jobdoc:param) &gt; 0][1]"
				mode="copy_node" />
	...
	<xsl:template
		match="jobdoc:configuration/jobdoc:params[not(@id = 'return_parameter')]"
		mode="copy_node">
		<params>
			<xsl:apply-templates select="@*" mode="copy_node" />
			<xsl:apply-templates select="note"
				mode="copy_node" />
			<xsl:apply-templates
				select="parent::jobdoc:configuration/jobdoc:params[not(@id = 'return_parameter')]//jobdoc:param"
				mode="copy_node" />
		</params>
	</xsl:template>
