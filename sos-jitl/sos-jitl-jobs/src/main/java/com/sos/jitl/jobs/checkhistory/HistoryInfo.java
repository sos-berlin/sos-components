...
public class HistoryInfo {
	...
	private HistoryItem executeApiCall(String query) throws Exception {
		...
		try {
			...
			case "lastcompletedsuccessful":
				historyItem.setResult((historyItem.getHistoryItemFound()
						&& historyItem.getState().get_text().value().equals(HistoryStateText.SUCCESSFUL.value())));
				break;
			case "lastcompletedfailed":
				historyItem.setResult((historyItem.getHistoryItemFound()
						&& historyItem.getState().get_text().value().equals(HistoryStateText.FAILED.value())));
				break;
			default:
				historyItem
						.setResult(parameterResolver.getCountResult(historyItem.getCount(), historyItem.getResult()));
			}
