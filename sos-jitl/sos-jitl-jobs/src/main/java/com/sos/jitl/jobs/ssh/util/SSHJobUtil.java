...
public class SSHJobUtil {
    ...
    public static void checkExitCode(Integer exitCode, SSHJobArguments jobArgs, Map<String, Object> outcomes, JobLogger logger) throws SOSJobSSHException {
        if (exitCode != null) {
            outcomes.put("exit_code", exitCode);
            if (!exitCode.equals(new Integer(0))) {
                if (jobArgs.getIgnoreError().getValue() || jobArgs.getExitCodesToIgnore().getValue().contains(exitCode)) {
