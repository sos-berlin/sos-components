...
public class YADEJobArguments extends JobArguments {
    ...
    /** Required */
    private JobArgument<Path> settings = new JobArgument<>(YADEArguments.ARG_SETTINGS, true);
    private JobArgument<String> profile = new JobArgument<>(YADEArguments.ARG_PROFILE, true);

    /** see {@link YADEArguments#ARG_PARALLELISM} */
    private JobArgument<Integer> parallelism = new JobArgument<>(YADEArguments.ARG_PARALLELISM, false, Integer.valueOf(1));

    /** Settings - overrides settings arguments */
    // Source
    private JobArgument<String> sourceDir = new JobArgument<>(YADEArguments.ARG_SOURCE_DIR, false);
    private JobArgument<String> sourceFilePath = new JobArgument<>(YADEArguments.ARG_SOURCE_FILE_PATH, false);
    private JobArgument<String> sourceFileSpec = new JobArgument<>(YADEArguments.ARG_SOURCE_FILE_SPEC, false);
    private JobArgument<Path> sourceFileList = new JobArgument<>(YADEArguments.ARG_SOURCE_FILE_LIST, false);
    private JobArgument<String> sourceExcludedDirectories = new JobArgument<>(YADEArguments.ARG_SOURCE_EXCLUDED_DIRECTORIES, false);
    // Target
    private JobArgument<String> targetDir = new JobArgument<>(YADEArguments.ARG_TARGET_DIR, false);

    /** Settings - replacement behavior */
    private JobArgument<Boolean> settingsReplacerCaseSensitive = new JobArgument<>(YADEArguments.ARG_SETTINGS_REPLACER_CASE_SENSITIVE, false,
            YADEArguments.ARG_SETTINGS_REPLACER_CASE_SENSITIVE_DEFAULT);
    private JobArgument<Boolean> settingsReplacerKeepUnresolved = new JobArgument<>(YADEArguments.ARG_SETTINGS_REPLACER_KEEP_UNRESOLVED, false,
