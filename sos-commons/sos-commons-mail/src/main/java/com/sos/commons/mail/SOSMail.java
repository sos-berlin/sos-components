...
public class SOSMail {
	...
	public SOSMail(final String smtpHost, final String smtpPort, final String smtpUser, final String smtpUserPassword)
			throws Exception {
		this.smtpProperties = new Properties();
		setHost(smtpHost);
		setPort(smtpPort);
	...
	private boolean sendJavaMail() throws Exception {
		...
		try {
			...
			lastError = String.format("%s while connecting to %s:%s %s /******** --> %s", ee.getClass().getSimpleName(),
					host, port, getUser(), ee.toString());
			...
			if (queueMailOnError) {
				if (!SOSString.isEmpty(queueDir) && e.getMessage().startsWith("Could not connect to SMTP host")
					...
					throw new Exception(
							String.format("%s occurred on send: %s", e.getClass().getSimpleName(), e.toString()), e);
				}
			} else {
				throw e;
			}
		...
		try {
			...
			if (!attachmentList.isEmpty() || !SOSString.isEmpty(alternativeBody)) {
				...
				if (this.haveAlternative()) {
					...
					if (contentType.startsWith("text/")) {
						alternativeBodypart.setContent(alternativeBody,
								alternativeContentType + ";charset= " + alternativeCharset);
					} else {
	...
	private void warn(final String n, final String v) {
		if (messageReady) {
			try {
				LOGGER.warn(
						"...setting of " + n + "=" + v + " will not be used. Loaded Message will be sent unchanged.");
			} catch (Exception e) {
