...
public class SOSPath {
	...
	public static void copyFile(final Path source, final Path dest, final boolean append) throws IOException {
		...
		try {
			...
			if (append) {
				out = Files.newOutputStream(dest, StandardOpenOption.CREATE, StandardOpenOption.WRITE,
	...
	public static Stream<Path> getFilesStream(final String folder, final String regexp, final int flag)
			throws IOException {
		return getFilesStream(folder, regexp, flag, false);
	}

	public static Stream<Path> getFilesStream(final Path folder, final String regexp, final int flag)
			throws IOException {
		return getFilesStream(folder, regexp, flag, false);
	}

	public static Stream<Path> getFilesStream(final String folder, final String regexp, final int flag,
			final boolean withSubFolder) throws IOException {
	...
	public static List<Path> getFileList(final String folder, final String regexp, final int flag,
			final boolean withSubFolder) throws IOException {
		return getFilesStream(folder, regexp, flag, withSubFolder).collect(Collectors.toList());
	}

	public static List<Path> getFileList(final Path folder, final String regexp, final int flag,
			final boolean withSubFolder) throws IOException {
		return getFilesStream(folder, regexp, flag, withSubFolder).collect(Collectors.toList());
	}
	...
	public static Stream<Path> getFolderStream(final String folder, final String regexp, final int flag)
			throws IOException {
		return getFolderStream(folder, regexp, flag, false);
	}

	public static Stream<Path> getFolderStream(final Path folder, final String regexp, final int flag)
			throws IOException {
		return getFolderStream(folder, regexp, flag, false);
	}
	...
	public static Stream<Path> getFolderStream(final Path folder, final String regexp, final int flag,
		...
		if (regexp != null) {
			final Pattern pattern = Pattern.compile(regexp, flag);
			predicate = p -> pattern.matcher(p.getFileName().toString()).find() && !".".equals(p.getFileName().toString())
	...
	public static List<Path> getFolderList(final String folder, final String regexp, final int flag)
			throws IOException {
	...
	public static List<Path> getFolderList(final Path folder, final String regexp, final int flag,
			final boolean withSubFolder) throws IOException {
		return getFolderStream(folder, regexp, flag, withSubFolder).collect(Collectors.toList());
	}
