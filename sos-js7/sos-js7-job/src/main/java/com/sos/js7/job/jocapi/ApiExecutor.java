...
public class ApiExecutor {
	...
	private static final List<String> DO_NOT_LOG_KEY = Arrays.asList(new String[] { "js7.api-server.password",
			"js7.api-server.cs-password", "js7.web.https.keystore.store-password",
			"js7.web.https.keystore.key-password", "js7.web.https.keystore.alias", "js7.web.https.truststores" });
	...
	private String getReasonPhrase() {
		String reasonPhrase = "";
		if (this.getClient() != null && this.getClient().getHttpResponse() != null
				&& this.getClient().getHttpResponse().getStatusLine() != null) {
		...
		for (String uri : jocUris) {
			try {
				...
				if (client.statusCode() == 401) {
					...
					return new ApiResponse(statusCode, getReasonPhrase(), response,
							client.getResponseHeader(ACCESS_TOKEN_HEADER), null);
				} else {
				...
				if (loginUri != null) {
					latestError = String.format("connection to URI %1$s: %2$s occurred: %3$s", loginUri, e.getClass(),
							e.getMessage());
					logDebug(latestError);
				} else {
	...
	public ApiResponse post(String token, String apiUrl, String body)
	...
	private List<String> getUris() throws SOSMissingDataException {
		...
		List<String> uris = config.getConfig(PRIVATE_CONF_JS7_PARAM_API_SERVER)
				.getStringList(PRIVATE_CONF_JS7_PARAM_URL);
	...
	private KeyStore readTrustStore(String path, KeystoreType type, String passwd) throws FileNotFoundException {
		if (!Files.exists(Paths.get(path))) {
			throw new FileNotFoundException(
					String.format("Cannot read from truststore %1$s. File does not exist!", path));
		}
		try {
	...
	private KeyStore readKeyStore(String path, KeystoreType type, String passwd) throws FileNotFoundException {
		if (!Files.exists(Paths.get(path))) {
			throw new FileNotFoundException(
					String.format("Cannot read from keystore %1$s. File does not exist!", path));
		}
		try {
	...
	private void applySSLContextCredentials(SOSRestApiClient client)
		...
		if (truststorePath != null && truststoreType != null && truststorePasswd != null) {
			KeyStore truststore = readTrustStore(truststorePath, KeystoreType.fromValue(truststoreType),
					truststorePasswd);
			if (truststore != null) {
				try {
	...
	private void tryCreateClient(String jocUri) throws SOSMissingDataException, KeyManagementException,
			SOSKeePassDatabaseException, NoSuchAlgorithmException, FileNotFoundException {
		if (client != null) {
	...
	private void setBasicAuthorizationIfExists(Config config)
			throws SOSKeePassDatabaseException, SOSMissingDataException {
		String csFile = null;
			...
			if (tokenJobResource.length == 2) {
				...
				if (detailValue != null) {
					if (detailValue.getSource().equals(jobResourceName)) {
						...
						LOGGER.info("Name of JobResource: " + detailValue.getSource() + " does not match the "
								+ tokenJobResource[0]);
					}
				}
			}
	...
	public void readConfig() throws SOSMissingDataException {
		...
		if (agentConfDirPath == null) {
			throw new SOSMissingDataException(String.format(
					"Environment variable %1$s not set. Can´t read credentials from agents private.conf file.",
		...
		Config defaultConfigWithAgentConfDir = ConfigFactory.parseProperties(props).withFallback(defaultConfig)
				.resolve();
		logDebug(PRIVATE_CONF_JS7_PARAM_CONFDIR + " (Config): "
				+ defaultConfigWithAgentConfDir.getString(PRIVATE_CONF_JS7_PARAM_CONFDIR));
		if (Files.exists(privatFolderPath.resolve(PRIVATE_CONF_FILENAME))) {
			config = ConfigFactory.parseFile(privatFolderPath.resolve(PRIVATE_CONF_FILENAME).toFile())
					.withFallback(defaultConfigWithAgentConfDir).resolve();
			if (config != null) {
			...
			throw new SOSMissingDataException(
					String.format("File %1$s does not exists. Can´t read credentials from agents private.conf file.",
	...
	private List<KeyStoreCredentials> readTruststoreCredentials(Config config) {
		...
		try {
			credentials = config.getConfigList(PRIVATE_CONF_JS7_PARAM_TRUSTORES_ARRAY).stream()
					.map(item -> new KeyStoreCredentials(
							item.getString(PRIVATE_CONF_JS7_PARAM_TRUSTSTORES_SUB_FILEPATH),
