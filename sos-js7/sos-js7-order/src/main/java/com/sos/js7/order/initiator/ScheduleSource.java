...
public abstract class ScheduleSource {
    ...
    public abstract List<Schedule> getSchedules() throws IOException, SOSHibernateException;

    public abstract String getSource();

    protected boolean checkMandatory(Schedule schedule) {
        if (schedule.getPath() == null) {
            LOGGER.warn("Adding order for workflow: " + schedule.getWorkflowPath() + " --> schedulePath: must not be null or empty.");
            return false;
        }
