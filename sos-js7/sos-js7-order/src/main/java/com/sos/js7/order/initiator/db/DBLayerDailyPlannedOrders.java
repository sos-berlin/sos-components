...
public class DBLayerDailyPlannedOrders {
	...
	public int deleteCascading(FilterDailyPlannedOrders filter) throws SOSHibernateException {
		...
		do {
			try {
				...
				LOGGER.debug("Retry delete orders as SOSHibernateLockAcquisitionException was thrown. Retry-counter: "
						+ retryCount);
	...
	private String getWhere(FilterDailyPlannedOrders filter, String pathField) {
		...
		if (filter.getOrderId() != null && !"".equals(filter.getOrderId())) {
			where += String.format(and + " p.orderId %s :orderId",
					SearchStringHelper.getSearchOperator(filter.getOrderId()));
			and = " and ";
		}
		...
		if (filter.getIsLate() != null) {
			if (filter.isLate()) {
				where += and + " (o.state is null and p.plannedStart < current_time()  or " + "o.state <> "
						+ DailyPlanOrderStateText.PLANNED.intValue() + " and o.startTime - p.plannedStart >= "
						+ DAILY_PLAN_LATE_TOLERANCE + ")  ";
			} else {
				where += and + " (o.state is null and p.plannedStart > current_time() or "
						+ "o.state is not null and o.state <> " + DailyPlanOrderStateText.PLANNED.intValue()
						+ " and o.startTime - p.plannedStart < " + DAILY_PLAN_LATE_TOLERANCE + ") ";
			}
			...
			for (DailyPlanOrderStateText state : filter.getStates()) {
				if (state.intValue() == DailyPlanOrderStateText.PLANNED.intValue()) {
					...
					if ((state.intValue() == DailyPlanOrderStateText.SUBMITTED.intValue())) {
						where += " ((o.state is null or o.state <> " + DailyPlanOrderStateText.FINISHED.intValue()
								+ ") and (p.submitted=1))" + " or";
					} else {
		...
		if (!"".equals(pathField) && filter.getSetOfScheduleFolders() != null
				&& filter.getSetOfScheduleFolders().size() > 0) {
			where += and + "(";
			for (Folder filterFolder : filter.getSetOfScheduleFolders()) {
				if (filterFolder.getRecursive()) {
					...
					where += " (" + "p.scheduleFolder" + " = '" + filterFolder.getFolder() + "' or "
							+ "p.scheduleFolder" + " like '" + likeFolder + "')";
				} else {
					where += String.format("p.scheduleFolder" + " %s '" + filterFolder.getFolder() + "'",
							SearchStringHelper.getSearchOperator(filterFolder.getFolder()));
				}
				where += " or ";
			}
			where += " 0=1)";
		...
		if (!"".equals(pathField) && filter.getSetOfWorkflowFolders() != null
				&& filter.getSetOfWorkflowFolders().size() > 0) {
			where += and + "(";
			for (Folder filterFolder : filter.getSetOfWorkflowFolders()) {
				if (filterFolder.getRecursive()) {
					String likeFolder = (filterFolder.getFolder() + "/%").replaceAll("//+", "/");
					where += " (" + "p.workflowFolder" + " = '" + filterFolder.getFolder() + "' or "
							+ "p.workflowFolder" + " like '" + likeFolder + "')";
				} else {
					where += String.format("p.workflowFolder" + " %s '" + filterFolder.getFolder() + "'",
							SearchStringHelper.getSearchOperator(filterFolder.getFolder()));
				}
				where += " or ";
			}
	...
	public List<DBItemDailyPlanWithHistory> getDailyPlanWithHistoryList(FilterDailyPlannedOrders filter,
			final int limit) throws SOSHibernateException {
		...
		if (filter.getListOfOrders() != null) {
			...
			if (size > SOSHibernate.LIMIT_IN_CLAUSE) {
				ArrayList<String> copy = (ArrayList<String>) filter.getListOfOrders().stream()
	...
	public List<DBItemDailyPlanOrders> getDailyPlanListExecute(FilterDailyPlannedOrders filter, final int limit)
			throws SOSHibernateException {
		String q = "from " + DBItemDailyPlannedOrders + " p " + getWhere(filter, "p.schedulePath")
	...
	public List<DBItemDailyPlanOrders> getDailyPlanList(FilterDailyPlannedOrders filter, final int limit)
			throws SOSHibernateException {
	...
	public Date getMaxPlannedStart(String jobschedulerId) {
		String q = "select max(plannedStart) from " + DBItemDailyPlannedOrders
				+ " where jobschedulerId=:jobschedulerId";
		Query<Date> query;
		try {
	...
	public DBItemDailyPlanOrders getUniqueDailyPlan(PlannedOrder plannedOrder)
		...
		LOGGER.trace("----> " + plannedOrder.getFreshOrder().getScheduledFor() + ":"
				+ new Date(plannedOrder.getFreshOrder().getScheduledFor()));
	...
	public void storeVariables(PlannedOrder plannedOrder, Long id)
	...
	public Long store(PlannedOrder plannedOrder, Long firstId, Integer nr, Integer size)
		...
		dbItemDailyPlannedOrders
				.setScheduleName(Paths.get(plannedOrder.getSchedule().getPath()).getFileName().toString());
		...
		String workflowFolder = Paths.get(plannedOrder.getSchedule().getWorkflowPath()).getParent().toString()
				.replace('\\', '/');
		...
		if (nr != 0) {
			...
			dbItemDailyPlannedOrders
					.setOrderId(dbItemDailyPlannedOrders.getOrderId().replaceAll("<nr.....>", nrAsString));
			dbItemDailyPlannedOrders
	...
	public int setSubmitted(FilterDailyPlannedOrders filter) throws SOSHibernateException {
		...
		if (filter.getSubmitted()) {
			...
			hql = "update  " + DBItemDailyPlannedOrders + " p set submitted=1,submitTime=:submitTime  "
					+ getWhere(filter);
	...
	public DBItemDailyPlanOrders insertFrom(DBItemDailyPlanOrders dbItemDailyPlanOrders) throws SOSHibernateException {
		...
		String newOrderId = DailyPlanHelper.modifiedOrderId(dbItemDailyPlanOrders.getOrderId(),
				dbItemDailyPlanOrders.getId());
	...
	public DBItemDailyPlanOrders addCyclicOrderIds(List<String> orderIds, String orderId, String controllerId,
			String timeZone, String periodBegin) throws SOSHibernateException {
		...
		try {
			...
			if (listOfPlannedOrders.size() == 1) {
				...
				if (dbItemDailyPlanOrder.getStartMode() == 1) {
					...
					filterCyclic.setDailyPlanDate(dbItemDailyPlanOrder.getDailyPlanDate(timeZone), timeZone,
							periodBegin);

					List<DBItemDailyPlanOrders> listOfPlannedCyclicOrders = dbLayerDailyPlannedOrders
							.getDailyPlanList(filterCyclic, 0);
