...
public class OrderInitiatorRunner extends TimerTask {
	...
	public OrderInitiatorRunner(List<ControllerConfiguration> controllers,
			OrderInitiatorSettings orderInitiatorSettings, boolean fromService) {
	...
	public Map<PlannedOrderKey, PlannedOrder> generateDailyPlan(String controllerId, JocError jocError,
			String accessToken, String dailyPlanDate, Boolean withSubmit)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
			DBMissingDataException, JocConfigurationException, DBOpenSessionException, IOException, ParseException,
			SOSException, URISyntaxException, ControllerConnectionResetException, ControllerConnectionRefusedException,
	...
	public void generateDailyPlan(String controllerId, String dailyPlanDate, Boolean withSubmit)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
			DBMissingDataException, JocConfigurationException, DBOpenSessionException,
			ControllerConnectionResetException, ControllerConnectionRefusedException, IOException, ParseException,
	...
	public void submitOrders(String controllerId, JocError jocError, String accessToken,
			List<DBItemDailyPlanOrders> listOfPlannedOrders)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
			DBMissingDataException, JocConfigurationException, DBOpenSessionException, IOException, ParseException,
			SOSException, URISyntaxException, ControllerConnectionResetException, ControllerConnectionRefusedException,
			InterruptedException, ExecutionException, TimeoutException {
		...
		try {
			...
			for (DBItemDailyPlanOrders dbItemDailyPlanOrders : listOfPlannedOrders) {
				...
				List<DBItemDailyPlanVariables> listOfOrderVariables = dbLayerOrderVariables
						.getOrderVariables(filterOrderVariables, 0);
				if (listOfOrderVariables != null && listOfOrderVariables.size() > 0
						&& listOfOrderVariables.get(0).getVariableValue() != null) {
					variables = Globals.objectMapper.readValue(listOfOrderVariables.get(0).getVariableValue(),
							Variables.class);
				}
				...
				FreshOrder freshOrder = buildFreshOrder(schedule, variableSet,
						dbItemDailyPlanOrders.getPlannedStart().getTime(), dbItemDailyPlanOrders.getStartMode(),
	...
	public void submitOrders(String controllerId, List<DBItemDailyPlanOrders> listOfPlannedOrders)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
			DBMissingDataException, JocConfigurationException, DBOpenSessionException,
			ControllerConnectionResetException, ControllerConnectionRefusedException, IOException, ParseException,
			SOSException, URISyntaxException, InterruptedException, ExecutionException, TimeoutException {
	...
	private List<DBItemDailyPlanSubmissions> getSubmissionsForDate(java.util.Calendar calendar, String controllerId)
		...
		try {
			...
			List<DBItemDailyPlanSubmissions> listOfDailyPlanSubmissions = dbLayerDailyPlan
					.getDailyPlanSubmissions(filter, 0);
...
//    public void createPlan(java.util.Calendar calendar)
    public void createPlan_new(java.util.Calendar calendar)
			...
			for (ControllerConfiguration controllerConfiguration : controllers) {
				...
				for (int day = 0; day < orderInitiatorSettings.getDayAheadPlan(); day++) {
					String dailyPlanDate = DailyPlanHelper.dateAsString(dailyPlanCalendar.getTime(),
							orderInitiatorSettings.getTimeZone());
					List<DBItemDailyPlanSubmissions> l = getSubmissionsForDate(dailyPlanCalendar,
							controllerConfiguration.getCurrent().getId());
					if ((l.size() == 0)) {
						if (logDailyPlan) {
							LOGGER.info("Creating daily plan for controller: "
									+ controllerConfiguration.getCurrent().getId() + " from " + dailyPlanDate + " for "
									+ orderInitiatorSettings.getDayAheadPlan() + " days ahead");

							LOGGER.info(
									"Submitting orders for controller " + controllerConfiguration.getCurrent().getId()
											+ " for " + orderInitiatorSettings.getDayAheadSubmit() + " days ahead");
						...
						LOGGER.info(
								"No orders will be created for " + dailyPlanDate + " as a submission has been found");
					}
		...
		} catch (SOSHibernateException | IOException | DBConnectionRefusedException | DBInvalidDataException
				| DBMissingDataException | JocConfigurationException | DBOpenSessionException e) {
...
//	public void createPlan_old_____first_create_then_submit(java.util.Calendar calendar)
	...
	private void submitDaysAhead(java.util.Calendar calendar, String controllerId)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
			DBMissingDataException, JocConfigurationException, DBOpenSessionException,
			ControllerConnectionResetException, ControllerConnectionRefusedException, IOException, ParseException,
			SOSException, URISyntaxException, InterruptedException, ExecutionException, TimeoutException {
		...
		try {
			...
			for (DBItemDailyPlanSubmissions dbItemDailyPlanSubmissions : listOfSubmissions) {
				...
				DBLayerDailyPlannedOrders dbLayerDailyPlannedOrders = new DBLayerDailyPlannedOrders(
						sosHibernateSession);
				List<DBItemDailyPlanOrders> listOfPlannedOrders = dbLayerDailyPlannedOrders.getDailyPlanList(filter, 0);

				OrderCounter o = DailyPlanHelper.getOrderCount(listOfPlannedOrders);

				LOGGER.info("Submitting " + o.getCount() + " orders " + o.cycledOrdersDesc() + " for controller "
						+ controllerId + " day: " + DailyPlanHelper.getDayOfYear(calendar));
	...
	public void run() {
		...
		java.util.Calendar calendar = DailyPlanHelper.getDailyplanCalendar(orderInitiatorSettings.getPeriodBegin(),
				orderInitiatorSettings.getTimeZone());
		calendar.add(java.util.Calendar.DATE, 1);

		java.util.Calendar now = java.util.Calendar
				.getInstance(TimeZone.getTimeZone(orderInitiatorSettings.getTimeZone()));

		if (startCalendar == null) {

			if (!"".equals(orderInitiatorSettings.getDailyPlanStartTime())) {
				startCalendar = DailyPlanHelper.getDailyplanCalendar(orderInitiatorSettings.getDailyPlanStartTime(),
						orderInitiatorSettings.getTimeZone());
			} else {
				startCalendar = DailyPlanHelper.getDailyplanCalendar(orderInitiatorSettings.getPeriodBegin(),
						orderInitiatorSettings.getTimeZone());
		...
		if (!createdPlans.contains(DailyPlanHelper.getDayOfYear(calendar))
				&& (generateFromManuelStart || (now.getTimeInMillis() - startCalendar.getTimeInMillis()) > 0)) {
			...
			} catch (ControllerConnectionResetException | ControllerConnectionRefusedException | ParseException
					| SOSException | URISyntaxException | InterruptedException | ExecutionException
					| TimeoutException e) {
				LOGGER.error(e.getMessage(), e);
			}
		}
	...
	private Calendar getCalendar(String controllerId, String calendarName, ConfigurationType type)
			throws DBMissingDataException, JsonParseException, JsonMappingException, IOException,
		...
		try {
			...
			DBLayerInventoryReleasedConfigurations dbLayer = new DBLayerInventoryReleasedConfigurations(
					sosHibernateSession);
			...
			if (config == null) {
				throw new DBMissingDataException(String.format("calendar '%s' not found for controller instance %s",
						calendarName, controllerId));
			}
	...
	private FreshOrder buildFreshOrder(Schedule schedule, VariableSet variableSet, Long startTime, Integer startMode,
			String timeZone, String periodBegin) {
		FreshOrder freshOrder = new FreshOrder();
		freshOrder.setId(
				DailyPlanHelper.buildOrderId(schedule, variableSet, startTime, startMode, timeZone, periodBegin));
	...
	private void generateNonWorkingDays(Date dailyPlanDate, Schedule o, String controllerId)
			...
			JocConfigurationException, DBOpenSessionException, SOSHibernateException {
		...
		if (o.getNonWorkingDayCalendars() != null) {
			...
			for (AssignedNonWorkingDayCalendars assignedNonWorkingCalendars : o.getNonWorkingDayCalendars()) {
				...
				calendarFilter
						.setDateFrom(DailyPlanHelper.dateAsString(dailyPlanDate, orderInitiatorSettings.getTimeZone()));
	...
	private OrderListSynchronizer calculateStartTimes(String controllerId, Date dailyPlanDate)
			throws JsonParseException, JsonMappingException, DBConnectionRefusedException, DBInvalidDataException,
		...
		try {
			...
			for (Schedule schedule : listOfSchedules) {
				if (fromService && !schedule.getPlanOrderAutomatically()) {
					LOGGER.debug(
							String.format("... schedule %s  will not be planned automatically", schedule.getPath()));
				} else {
					...
					for (AssignedCalendars assignedCalendar : schedule.getCalendars()) {
						...
						CalendarCacheItem calendarCacheItem = calendarCache
								.get(assignedCalendar.getCalendarName() + "#" + schedule.getPath());
						...
						if (calendarCacheItem == null) {
							...
							Calendar calendar = getCalendar(controllerId, assignedCalendar.getCalendarName(),
									ConfigurationType.WORKINGDAYSCALENDAR);
							calendarCacheItem.calendar = calendar;
							calendarCache.put(assignedCalendar.getCalendarName() + "#" + schedule.getPath(),
									calendarCacheItem);
						} else {
						...
						List<String> dates = fr
								.resolveRestrictions(calendarJson, restrictionJson, actDateAsString, nextDateAsString)
								.getDates();
						...
						for (Period p : assignedCalendar.getPeriods()) {
							...
							periodResolver.addStartTimes(_period, dailyPlanDateAsString,
						...
						for (String d : dates) {
							...
							if (listOfNonWorkingDays != null && listOfNonWorkingDays.get(d) != null) {
								...
								Map<Long, Period> listOfStartTimes = periodResolver.getStartTimes(d,
										dailyPlanDateAsString, assignedCalendar.getTimeZone());
								for (Entry<Long, Period> periodEntry : listOfStartTimes.entrySet()) {
									...
									for (VariableSet variableSet : schedule.getVariableSets()) {
										FreshOrder freshOrder = buildFreshOrder(schedule, variableSet,
												...
												this.orderInitiatorSettings.getTimeZone(),
												this.orderInitiatorSettings.getPeriodBegin());
										...
										if (!fromService) {
											schedule.setSubmitOrderToControllerWhenPlanned(
													orderInitiatorSettings.isSubmit());
										}
										plannedOrder.setSchedule(schedule);
										if (variableSet.getOrderName() != null
												&& !variableSet.getOrderName().isEmpty()) {
											plannedOrder.setOrderName(variableSet.getOrderName());
										} else {
											plannedOrder.setOrderName(
													Paths.get(schedule.getPath()).getFileName().toString());
										}
	...
	private DBItemDailyPlanSubmissions addDailyPlanSubmission(String controllerId, Date dateForPlan)
		...
		try {
			sosHibernateSession = Globals.createSosHibernateStatelessConnection("OrderInitiatorRunner");
			DBLayerDailyPlanSubmissions dbLayerDailyPlanSubmissions = new DBLayerDailyPlanSubmissions(
					sosHibernateSession);
			...
			dbLayerDailyPlanSubmissions.storeSubmission(dbItemDailyPlanSubmissionHistory,
					orderInitiatorSettings.getSubmissionTime());
