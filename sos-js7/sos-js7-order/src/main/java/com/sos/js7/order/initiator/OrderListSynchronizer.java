...
public class OrderListSynchronizer {
	...
	public boolean add(String controllerId, PlannedOrder o) {
		...
		if (listOfExistingWorkflows.get(w) == null) {
			listOfExistingWorkflows.put(w,
					WorkflowsHelper.workflowCurrentlyExists(currentState, o.getSchedule().getWorkflowName()));
			LOGGER.debug("Adding workflow " + w.workflowName + " for controller " + w.controllerId
					+ " to list of existing workflows");
		}
	...
	private List<DBItemDailyPlanHistory> insertHistory(SOSHibernateSession sosHibernateSession,
		...
		for (PlannedOrder addedOrder : addedOrders) {
			...
			if (dbItemDailyPlanHistory.getWorkflowPath() != null) {
				String folderName = Paths.get(dbItemDailyPlanHistory.getWorkflowPath()).getParent().toString()
						.replace('\\', '/');
	...
	public void submitOrdersToController(String controllerId)
			throws ControllerConnectionResetException, ControllerConnectionRefusedException, DBMissingDataException,
	...
	private void executeStore() throws JocConfigurationException, DBConnectionRefusedException, SOSHibernateException,
	...
	public void addPlannedOrderToControllerAndDB(String controllerId, Boolean withSubmit)
			throws JocConfigurationException, DBConnectionRefusedException, ControllerConnectionResetException,
			ControllerConnectionRefusedException, DBMissingDataException, DBOpenSessionException,
		...
		try {
			...
			if (orderInitiatorSettings.isOverwrite()) {
				...
				for (PlannedOrder plannedOrder : listOfPlannedOrders.values()) {
					...
					String workflowName = Paths.get(plannedOrder.getFreshOrder().getWorkflowPath()).getFileName()
							.toString();
				...
				CompletableFuture<Either<Problem, Void>> c = OrdersHelper.removeFromJobSchedulerController(controllerId,
						listOfDailyPlanOrders);
				c.thenAccept(either -> {
					...
					if (either.isRight()) {
						...
						try {
							...
							DBLayerDailyPlannedOrders dbLayerDailyPlannedOrders2 = new DBLayerDailyPlannedOrders(
									sosHibernateSession2);
							for (PlannedOrder plannedOrder : listOfPlannedOrders.values()) {
								final FilterDailyPlannedOrders filter = new FilterDailyPlannedOrders();
								filter.setPlannedStart(new Date(plannedOrder.getFreshOrder().getScheduledFor()));
								LOGGER.trace("----> Remove: " + plannedOrder.getFreshOrder().getScheduledFor() + ":"
										+ new Date(plannedOrder.getFreshOrder().getScheduledFor()));
								filter.setControllerId(controllerId);
								String workflowName = Paths.get(plannedOrder.getFreshOrder().getWorkflowPath())
						...
						} catch (SOSHibernateException | JocConfigurationException | DBConnectionRefusedException
								| ParseException | ControllerConnectionResetException
								| ControllerConnectionRefusedException | DBMissingDataException | DBOpenSessionException
								| DBInvalidDataException | InterruptedException | ExecutionException
								| TimeoutException e) {
							ProblemHelper.postExceptionEventIfExist(Either.left(e), getAccessToken(), getJocError(),
									controllerId);
