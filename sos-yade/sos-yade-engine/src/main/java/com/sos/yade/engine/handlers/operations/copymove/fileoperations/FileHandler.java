...
public class FileHandler {
    ...
    public static void process(ISOSLogger logger, CopyMoveOperationsConfig config, YADESourceProviderDelegator sourceDelegator,
        ...
        MessageDigest targetMessageDigest = FileIntegrityHashHelper.getMessageDigest(config, config.getTarget().isCreateIntegrityHashFileEnabled());
        ...
        l: while (attempts < config.getMaxRetries()) {
            ...
            try (InputStream sourceStream = FileStreamHelper.getSourceInputStream(config, sourceDelegator, sourceFile, useBufferedStreams);
                    OutputStream targetOutputStream = FileStreamHelper.getTargetOutputStream(config, targetDelegator, targetFile, useBufferedStreams);
                    OutputStream targetStream = isCompressTarget ? new GZIPOutputStream(targetOutputStream) : targetOutputStream) {
                if (attempts > 0) {
