...
public class YADECommandExecutor {
    ...
    public static void executeBeforeOperation(ISOSLogger logger, AYADEProviderDelegator delegator, RetryOnConnectionError retry,
        ...
        try {
            ...
            throw new YADEEngineCommandException(e.toString(), e, delegator);
        }
        ...
        for (String command : arg.getValue()) {
            ...
            if (isJumpHostClientCommand) {
                ...
                try {
                    checkCommandResult(delegator, null, argumentName, result);
                } catch (YADEEngineCommandException e) {
                    throw new YADEEngineJumpHostCommandException(e, delegator);
                }
            } else {
                logCommandResult(logger, "[" + delegator.getLabel() + "][" + argumentName + "]", result);
                checkCommandResult(delegator, null, argumentName, result);
    ...
    public static YADECommandResult executeAfterOperationOnError(ISOSLogger logger, YADESourceProviderDelegator sourceDelegator,
            YADETargetProviderDelegator targetDelegator, Throwable exception, RetryOnConnectionError retry) {
    ...
    public static void executeBeforeFile(ISOSLogger logger, YADESourceProviderDelegator sourceDelegator, YADETargetProviderDelegator targetDelegator,
        ...
        if (!arg.isEmpty()) {
            ...
            if (!skipped || sourceDelegator.getArgs().getCommands().getCommandsBeforeFileEnableForSkipped().isTrue()) {
        ...
        if (targetDelegator != null) {
            ...
            if (!arg.isEmpty() && targetFile != null) {
                if (!targetFile.isSkipped() || targetDelegator.getArgs().getCommands().getCommandsBeforeFileEnableForSkipped().isTrue()) {
    ...
    public static void executeJumpHostCommand(ISOSLogger logger, AYADEProviderDelegator jumpHostDelegator, String command)
        ...
        try {
            checkCommandResult(jumpHostDelegator, null, null, result);
        } catch (YADEEngineCommandException e) {
            throw new YADEEngineJumpHostCommandException(e, jumpHostDelegator);
        }
    }
    ...
    private static void executeFileCommands(ISOSLogger logger, IYADEProviderDelegator delegator, YADESourceProviderDelegator sourceDelegator,
        ...
        for (String command : arg.getValue()) {
            ...
            checkCommandResult(delegator, prefix, null, result);
        }
    }
    ...
    private static void checkCommandResult(IYADEProviderDelegator delegator, String prefix, String argumentName, SOSCommandResult result)
            throws YADEEngineCommandException {
        if (result == null) {
            return;
        }
        if (result.hasError(THROW_ERROR_ON_STDERR)) {
            ...
            String px = prefix;
            if (px == null) {
                px = "[" + delegator.getLabel() + "]";
            }
            if (argumentName != null) {
                px += "[" + argumentName + "]";
            }
            throw new YADEEngineCommandException(px, result.getExitCode(), std, delegator);
        }
    }
    ...
    public class YADECommandResult {
        ...
        public YADEEngineCommandException getSourceException() {
            return source;
        }

        public YADEEngineCommandException getTargetException() {
            return target;
        }
