...
public class InventorySearchDBLayerTest {
    ...
    @Ignore
    @Test
    public void testBasicSearchInventoryCalendarsConfigurations() throws Exception {
        SOSHibernateFactory factory = null;
        SOSHibernateSession session = null;
        try {
            String search = null;
            ...
            List<InventorySearchItem> items = dbLayer.getBasicSearchInventoryConfigurations(RequestSearchReturnType.CALENDAR, search, folders, tags,
                    null, null);
            LOGGER.info("[CALENDAR]RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
            }

            items = dbLayer.getBasicSearchInventoryConfigurations(RequestSearchReturnType.WORKINGDAYSCALENDAR, search, folders, tags, null, null);
            LOGGER.info("[WORKINGDAYSCALENDAR]RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
            }

            items = dbLayer.getBasicSearchInventoryConfigurations(RequestSearchReturnType.NONWORKINGDAYSCALENDAR, search, folders, tags, null, null);
            LOGGER.info("[NONWORKINGDAYSCALENDAR]RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
            }
    ...
    public void testAdvancedSearchInventoryConfigurations() throws Exception {
        ...
        try {
            ...
            List<InventorySearchItem> items = dbLayer.getAdvancedSearchInventoryConfigurations(RequestSearchReturnType.NONWORKINGDAYSCALENDAR, search,
                    folders, tags, unDeployedUnReleaseObjects, valid, advanced);
    ...
    public void testAdvancedSearchDeployedOrReleasedConfigurations() throws Exception {
        ...
        try {
            ...
            List<InventorySearchItem> items = dbLayer.getAdvancedSearchDeployedOrReleasedConfigurations(RequestSearchReturnType.WORKINGDAYSCALENDAR,
                    search, folders, tags, advanced, "js7.x");
            LOGGER.info("RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
            }
    ...
    @Ignore
    @Test
    public void testAdvancedSearchDeployedOrReleasedCalendarConfigurations() throws Exception {
        SOSHibernateFactory factory = null;
        SOSHibernateSession session = null;
        try {
            String search = null;
            ...
            items = dbLayer.getAdvancedSearchDeployedOrReleasedConfigurations(RequestSearchReturnType.WORKINGDAYSCALENDAR, search, folders, tags,
                    advanced, "js7.x");
            LOGGER.info("[WORKINGDAYSCALENDAR]RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
            ...
            items = dbLayer.getAdvancedSearchDeployedOrReleasedConfigurations(RequestSearchReturnType.NONWORKINGDAYSCALENDAR, search, folders, tags,
                    advanced, "js7.x");
            LOGGER.info("[NONWORKINGDAYSCALENDAR]RESULT=" + items.size());
            for (InventorySearchItem item : items) {
                LOGGER.info(SOSString.toString(item));
