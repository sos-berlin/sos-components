		...
		try {
			...
			if (dbItem != null) {
				...
				if (ldapServerUrl == null || ldapServerUrl.isEmpty()) {
					ldapServerUrl = getProperty(properties.getLdap().getExpert().getIamLdapServerUrl(),
							"http://localhost:389");
				}

				if (userDnTemplate == null || userDnTemplate.isEmpty()) {
					userDnTemplate = getProperty(properties.getLdap().getExpert().getIamLdapUserDnTemplate(),
				...
				if (truststoreType == null && properties.getLdap().getExpert().getIamLdapTruststoreType() != null
						&& !properties.getLdap().getExpert().getIamLdapTruststoreType().isEmpty()) {
					truststoreType = KeystoreType
							.fromValue(getProperty(properties.getLdap().getExpert().getIamLdapTruststoreType(), ""));
				}

				if (groupRolesMap == null) {
					if (properties.getLdap().getExpert().getIamLdapGroupRolesMap() != null
							&& properties.getLdap().getExpert().getIamLdapGroupRolesMap().getItems() != null) {
						groupRolesMap = new HashMap<String, List<String>>();
						for (LdapGroupRolesMappingItem entry : properties.getLdap().getExpert()
								.getIamLdapGroupRolesMap().getItems()) {
