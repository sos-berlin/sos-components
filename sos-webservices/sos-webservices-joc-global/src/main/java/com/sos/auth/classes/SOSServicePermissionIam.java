...
public class SOSServicePermissionIam {
	...
	public JOCDefaultResponse postJocCockpitPermissions(@HeaderParam(X_ACCESS_TOKEN) String accessToken) {
		...
		try {
			...
			SOSAuthCurrentAccount currentAccount = getCurrentAccount(
			...
			return JOCDefaultResponse.responseStatus200(
					Globals.objectMapper.writeValueAsBytes(currentAccount.getSosPermissionJocCockpitControllers()));
		} catch (org.apache.shiro.session.ExpiredSessionException e) {
			SOSAuthCurrentAccountAnswer sosAuthCurrentAccountAnswer = createSOSAuthCurrentAccountAnswer(accessToken,
					sosWebserviceAuthenticationRecord.getAccount(), e.getMessage());
		...
		try {
			if (Globals.jocWebserviceDataContainer.getCurrentAccountsList() == null) {
				...
				return JOCDefaultResponse
						.responseStatus200(Globals.jocWebserviceDataContainer.getCurrentAccountsList().size());
			}
		} catch (Exception e) {
			return JOCDefaultResponse.responseStatusJSError(e);
		} finally {
	...
	public JOCDefaultResponse getAccessToken(String account) {
		...
		try {
			if (Globals.jocWebserviceDataContainer.getCurrentAccountsList() != null) {
				SOSAuthCurrentAccountAnswer s = Globals.jocWebserviceDataContainer.getCurrentAccountsList()
	...
	public JOCDefaultResponse loginPost(@Context HttpServletRequest request,
			@HeaderParam("Authorization") String basicAuthorization, @HeaderParam("X-CLIENT-ID") String loginClientId,
			@QueryParam("user") String account, @QueryParam("pwd") String pwd) {
	...
	private void removeTimeOutShiroDBSessions()
			throws FileNotFoundException, SOSHibernateException, JocConfigurationException, DBOpenSessionException {
		if (Globals.shiroIsActive) {
			...
			try {
				...
				List<DBItemJocConfiguration> listOfConfigurtions = jocConfigurationDBLayer
	...
	protected JOCDefaultResponse logout(String accessToken) {
		...
		SOSAuthCurrentAccount currentAccount = Globals.jocWebserviceDataContainer.getCurrentAccountsList()
	...
	public JOCDefaultResponse isPermitted(@HeaderParam(X_ACCESS_TOKEN) String xAccessTokenFromHeader,
			@QueryParam(ACCESS_TOKEN) String accessTokenFromQuery, @QueryParam("permission") String permission)
	...
	private SOSAuthCurrentAccount getCurrentAccount(String accessToken) throws SessionNotExistException {
		...
		if (Globals.jocWebserviceDataContainer.getCurrentAccountsList() != null && accessToken != null
	...
	private String createAccount(SOSAuthCurrentAccount currentAccount, String password,
			DBItemIamIdentityService dbItemIdentityService) throws Exception {
		if (Globals.jocWebserviceDataContainer.getCurrentAccountsList() == null) {
			Globals.jocWebserviceDataContainer.setCurrentAccountsList(new SOSAuthCurrentAccountsList());
		}

		IdentityServiceTypes identityServiceType = IdentityServiceTypes
				.fromValue(dbItemIdentityService.getIdentityServiceType());
		...
		currentAccount.setIdentityServices(new SOSIdentityService(dbItemIdentityService.getId(),
				dbItemIdentityService.getIdentityServiceName(), identityServiceType));
	...
	private String getPwdFromHeaderOrQuery(String basicAuthorization, String pwd)
			throws UnsupportedEncodingException, JocException {
		String authorization = EMPTY_STRING;
	...
	private SOSAuthCurrentAccountAnswer authenticate(SOSAuthCurrentAccount currentAccount, String password)
			throws Exception {
		...
		try {
			...
			try {
				...
				for (DBItemIamIdentityService dbItemIamIdentityService : listOfIdentityServices) {
					...
					SecurityConfiguration securityConfiguration = sosPermissionMerger
							.addIdentityService(new SOSIdentityService(dbItemIamIdentityService));
					...
					if (!msg.isEmpty()) {
						LOGGER.info("Login with required Identity Service "
								+ dbItemIamIdentityService.getIdentityServiceName() + " failed." + msg);
					}
				...
				if (currentAccount.getCurrentSubject() == null) {
					...
					for (DBItemIamIdentityService dbItemIamIdentityService : listOfIdentityServices) {
						try {
							...
							if (msg.isEmpty()) {
								LOGGER.info("Login with Identity Service "
										+ dbItemIamIdentityService.getIdentityServiceName() + " successful.");
								addFolder(currentAccount);
								break;
							}
							...
							LOGGER.info("Login with Identity Service "
									+ dbItemIamIdentityService.getIdentityServiceName() + " failed.");
							msg = e.getMessage();
							continue;
						}
					}
				...
				if (currentAccount.getCurrentSubject() != null) {
					...
					SOSPermissionsCreator sosPermissionsCreator = new SOSPermissionsCreator(currentAccount);
					Permissions sosPermissionJocCockpitControllers = sosPermissionsCreator
							.createJocCockpitPermissionControllerObjectList(securityConfigurationEntry);
			...
			if (currentAccount.getCurrentSubject() == null || !currentAccount.getCurrentSubject().isAuthenticated()) {
				...
				if (currentAccount.getIdentityServices() != null) {
					sosAuthCurrentUserAnswer
							.setIdentityService(currentAccount.getIdentityServices().getIdentityServiceName());
				} else {
			...
			sosAuthCurrentUserAnswer
					.setIsForcePasswordChange(currentAccount.getCurrentSubject().isForcePasswordChange());
			...
			sosAuthCurrentUserAnswer.setIdentityService(currentAccount.getIdentityServices().getIdentyServiceType()
					+ ":" + currentAccount.getIdentityServices().getIdentityServiceName());
			sosAuthCurrentUserAnswer.setMessage(msg);

			LOGGER.debug("CallerIpAddress=" + currentAccount.getCallerIpAddress());

			boolean enableTouch = "true".equals(Globals.sosCockpitProperties.getProperty(
	...
	protected JOCDefaultResponse login(HttpServletRequest request, String basicAuthorization, String clientCertCN,
		...
		try {
			...
			JocConfigurationDbLayer jocConfigurationDBLayer = new JocConfigurationDbLayer(sosHibernateSession);
			List<DBItemJocConfiguration> listOfDbItemJocConfiguration = jocConfigurationDBLayer
					.getJocConfigurations(filter, 0);
			if (listOfDbItemJocConfiguration.size() == 1) {
				...
				com.sos.joc.model.security.Properties properties = Globals.objectMapper
						.readValue(dbItem.getConfigurationItem(), com.sos.joc.model.security.Properties.class);
		...
		if (Globals.jocWebserviceDataContainer.getCurrentAccountsList() != null) {
			Globals.jocWebserviceDataContainer.getCurrentAccountsList()
					.removeTimedOutAccount(currentAccount.getAccountname());
		...
		if (!sosAuthCurrentUserAnswer.isAuthenticated()) {
			...
			SOSSessionHandler sosSessionHandler = new SOSSessionHandler(currentAccount);
			return JOCDefaultResponse.responseStatus200WithHeaders(sosAuthCurrentUserAnswer,
					sosAuthCurrentUserAnswer.getAccessToken(), sosSessionHandler.getTimeout());
		}
	...
	private SOSAuthCurrentAccountAnswer createSOSAuthCurrentAccountAnswer(String accessToken, String user,
			String message) {
