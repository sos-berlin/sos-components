...
public class JocAuditLog {
    ...
    public static void storeAuditLogDetails(Stream<AuditLogDetail> details, SOSHibernateSession connection, Long auditlogId, Date now) {
        if (details != null && auditlogId != null) {
            if (connection == null) {
                ...
                Stream<DBItemJocAuditLogDetails> dbDetails = details.map(detail -> storeAuditLogDetail(detail.getAuditLogDetail(auditlogId),
                        connection));
    ...
    public static DBItemJocAuditLogDetails storeAuditLogDetail(AuditLogDetail detail, SOSHibernateSession connection, DBItemJocAuditLog dbAudit) {
        return storeAuditLogDetail(detail, connection, dbAudit.getId());
    }

    private static DBItemJocAuditLogDetails storeAuditLogDetail(AuditLogDetail detail, SOSHibernateSession connection, Long auditlogId) {
        if (detail != null && auditlogId != null) {
            if (connection == null) {
                try {
                    connection = Globals.createSosHibernateStatelessConnection("storeAuditLogDetail");
                    return storeAuditLogDetail(detail.getAuditLogDetail(auditlogId), connection);
                } catch (Exception e) {
                    LOGGER.error("", e);
                } finally {
                    Globals.disconnect(connection);
