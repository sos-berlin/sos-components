...
public class IamIdentityServiceDBLayer {
	...
	private static final String DBItemIamIdentityService = com.sos.joc.db.authentication.DBItemIamIdentityService.class
			.getSimpleName();
	...
	public DBItemIamIdentityService getUniqueIdentityService(IamIdentityServiceFilter filter)
			throws SOSHibernateException {
		List<DBItemIamIdentityService> identityList = null;
		Query<DBItemIamIdentityService> query = sosHibernateSession.createQuery("from " + DBItemIamIdentityService
				+ getWhere(filter) + filter.getOrderCriteria() + filter.getSortMode());
	...
	public List<DBItemIamIdentityService> getIdentityServiceList(IamIdentityServiceFilter filter, final int limit)
			throws SOSHibernateException {
	...
	private void deleteInVault(Set<String> setOfAccounts, Long identityServiceId, String identityServiceName,
		...
		SOSIdentityService sosIdentityService = new SOSIdentityService(identityServiceId, identityServiceName,
				identityServiceType);
			...
			for (DBItemIamIdentityService dbItemIamIdentityService : identityServices2BeDeleted) {
				...
				try {
					if (IdentityServiceTypes.VAULT_JOC_ACTIVE.toString()
						...
						deleteInVault(accounts, dbItemIamIdentityService.getId(),
				...
				List<DBItemJocConfiguration> listOfdbItemJocConfiguration = jocConfigurationDBLayer
						.getJocConfigurations(jocConfigurationFilter, 0);
