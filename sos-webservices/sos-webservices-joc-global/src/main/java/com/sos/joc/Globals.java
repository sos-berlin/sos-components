...
public class Globals {
	...
	public static IniSecurityManagerFactory getShiroIniSecurityManagerFactory() {
		...
		if (factory == null) {
			...
			if (shiroNeedFactoryReloadAfterIniChanged(oldShiroIni, currentShiroIni)
					&& !oldShiroIni.equals(currentShiroIni)) {
	...
	public static Path getHibernateConfFile() throws JocConfigurationException {
		...
		if (p != null) {
			...
			throw new JocConfigurationException(
					String.format("hibernate configuration (%1$s) is set but file not found.", confFile));
		}
		return p;
	}
		...
		if (Globals.jocSecurityLevel == null) {
			...
			try {
				...
				if (stream != null) {
					...
					try {
						Globals.jocSecurityLevel = JocSecurityLevel.fromValue(
								properties.getProperty("security_level", defaultLevel.value()).toUpperCase());
					} catch (Exception e) {
						//
					}
				}
		...
		try {
			if (servletBaseUri == null) {
				if (servletContextContextPath == null) {
					...
					LOGGER.debug(String.format("servletContextContextPath=%s, baseUri=%s", servletContextContextPath,
							baseUri));
	...
	public static ConfigurationGlobalsJoc getConfigurationGlobalsJoc() {
		return Globals.configurationGlobals == null ? new ConfigurationGlobalsJoc()
				: (ConfigurationGlobalsJoc) Globals.configurationGlobals.getConfigurationSection(DefaultSections.joc);
	}

	public static ConfigurationGlobalsUser getConfigurationGlobalsUser() {
		return Globals.configurationGlobals == null ? new ConfigurationGlobalsUser()
				: (ConfigurationGlobalsUser) Globals.configurationGlobals.getConfigurationSection(DefaultSections.user);
	}
	...
	public static ConfigurationGlobalsDailyPlan getConfigurationGlobalsDailyPlan() {
		return Globals.configurationGlobals == null ? new ConfigurationGlobalsDailyPlan()
				: (ConfigurationGlobalsDailyPlan) Globals.configurationGlobals
						.getConfigurationSection(DefaultSections.dailyplan);
	}

	public static ConfigurationGlobalsGit getConfigurationGlobalsGit() {
		return Globals.configurationGlobals == null ? new ConfigurationGlobalsGit()
				: (ConfigurationGlobalsGit) Globals.configurationGlobals.getConfigurationSection(DefaultSections.git);
	}
	...
	public static int curVersionCompareWith(String version) {
		try {
			...
			curVersionsStr += curVersionsComplete[1].replaceFirst("SNAPSHOT", "00").replaceFirst("RC(\\d)", "$1")
					.replaceFirst(" ", "99").replaceFirst("^(\\d)$", "0$1");
			for (int i = 1; i < versions.length; i++) {
				versionsStr += versions[i].replaceFirst("^(\\d)$", "0$1");
			}
			versionsStr += versionsComplete[1].replaceFirst("SNAPSHOT", "00").replaceFirst("RC(\\d)", "$1")
					.replaceFirst(" ", "99").replaceFirst("^(\\d)$", "0$1");
