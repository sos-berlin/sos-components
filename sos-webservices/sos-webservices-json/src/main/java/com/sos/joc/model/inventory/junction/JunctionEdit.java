
package com.sos.joc.model.inventory.junction;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonPropertyDescription;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.sos.inventory.model.junction.Junction;
import com.sos.joc.model.inventory.ConfigurationObject;
import org.apache.commons.lang.builder.EqualsBuilder;
import org.apache.commons.lang.builder.HashCodeBuilder;
import org.apache.commons.lang.builder.ToStringBuilder;


/**
 * JS Junction Edit configuration
 * <p>
 * 
 * 
 */
@JsonInclude(JsonInclude.Include.NON_EMPTY)
@JsonPropertyOrder({
    "configuration"
})
public class JunctionEdit
    extends ConfigurationObject
{

    /**
     * junction
     * <p>
     * deploy object with fixed property 'TYPE':'Junction'
     * 
     */
    @JsonProperty("configuration")
    @JsonPropertyDescription("deploy object with fixed property 'TYPE':'Junction'")
    private Junction configuration;

    /**
     * junction
     * <p>
     * deploy object with fixed property 'TYPE':'Junction'
     * 
     */
    @JsonProperty("configuration")
    public Junction getConfiguration() {
        return configuration;
    }

    /**
     * junction
     * <p>
     * deploy object with fixed property 'TYPE':'Junction'
     * 
     */
    @JsonProperty("configuration")
    public void setConfiguration(Junction configuration) {
        this.configuration = configuration;
    }

    @Override
    public String toString() {
        return new ToStringBuilder(this).appendSuper(super.toString()).append("configuration", configuration).toString();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder().appendSuper(super.hashCode()).append(configuration).toHashCode();
    }

    @Override
    public boolean equals(Object other) {
        if (other == this) {
            return true;
        }
        if ((other instanceof JunctionEdit) == false) {
            return false;
        }
        JunctionEdit rhs = ((JunctionEdit) other);
        return new EqualsBuilder().appendSuper(super.equals(other)).append(configuration, rhs.configuration).isEquals();
    }

}
