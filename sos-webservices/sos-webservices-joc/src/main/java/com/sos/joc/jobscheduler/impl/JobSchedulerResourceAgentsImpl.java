    ...
    public JOCDefaultResponse postJobschedulerAgents(String accessToken, AgentFilter agentFilter) {
        try {
            JOCDefaultResponse jocDefaultResponse = init(API_CALL, agentFilter, accessToken, agentFilter.getJobschedulerId(), getPermissonsJocCockpit(
                    agentFilter.getJobschedulerId(), accessToken).getJobschedulerUniversalAgent().getView().isStatus());
            ...
            if (agentFilter.getAgents() != null && agentFilter.getAgents().size() > 0) {
                ...
                for (AgentUrl agentUri : agentUris) {
                    tasks.add(new AgentVCallable(agentUri.getAgent(), new JOCJsonCommand(this)));
                }
            } else {
                ...
                for (JsonString agentUri : agentUris.getValuesAs(JsonString.class)) {
                    tasks.add(new AgentVCallable(agentUri.getString(), new JOCJsonCommand(this)));
                }
            }
            if (!tasks.isEmpty()) {
