...
public class StoreNotificationImpl extends JOCResourceImpl implements IStoreNotification {
    ...
    public JOCDefaultResponse postStoreNotification(String xAccessToken, byte[] storeNotificationFilter) {
        ...
        try {
            ...
            if (item == null) {
                item = StoreResourceImpl.create(hibernateSession, in, name, getAccountName(), dbAuditlog.getId());
            } else {
                ...
                if (isChanged) {
                    item = StoreResourceImpl.update(hibernateSession, in, item, name, getAccountName(), dbAuditlog.getId());
                }
            }
