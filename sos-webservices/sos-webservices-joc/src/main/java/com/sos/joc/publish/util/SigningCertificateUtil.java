...
public class SigningCertificateUtil {
    ...
    public static JocKeyPair createSigningKeyPair (SOSHibernateSession hibernateSession, CreateCSRFilter createCsrFilter, String keyAlgorithm, Date validUntil)
        ...
        if(SOSKeyConstants.RSA_ALGORITHM_NAME.equals(keyAlgorithm)) {
            ...
            PKCS10CertificationRequest csr = CAUtils.createCSR(signerAlgorithm, newClientKeyPair, userDN);
            clientCert = CAUtils.signSigningCSR(signerAlgorithm, KeyUtil.getKeyPairFromRSAPrivatKeyString(rootKeyPair.getPrivateKey()).getPrivate(),
                    csr, rootCert, createCsrFilter.getSan(), validUntil);
            ...
            PKCS10CertificationRequest csr = CAUtils.createCSR(signerAlgorithm, newClientKeyPair, userDN);
            clientCert = CAUtils.signSigningCSR(signerAlgorithm, KeyUtil.getKeyPairFromECDSAPrivatKeyString(rootKeyPair.getPrivateKey()).getPrivate(),
                    csr, rootCert, createCsrFilter.getSan(), validUntil);
