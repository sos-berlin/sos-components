		...
		try {
			...
			if (calendarsFilter.getCalendars() == null || calendarsFilter.getCalendars().size() == 0) {
				for (Long calendarId : new HashSet<Long>(calendarsFilter.getCalendarIds())) {
					executeDeleteCalendar(connection, calendarDbLayer.getCalendar(calendarId), calendarsFilter,
							calendarDbLayer, accessToken);
				}
			} else {
				for (String calendarPath : new HashSet<String>(calendarsFilter.getCalendars())) {
					executeDeleteCalendar(connection,
							calendarDbLayer.getCalendar(dbItemInventoryInstance.getSchedulerId(), calendarPath),
							calendarsFilter, calendarDbLayer, accessToken);
				}
			}
			if (eventCommands != null && !eventCommands.isEmpty()
					&& "active".equals(dbItemInventoryInstance.getClusterType())) {
